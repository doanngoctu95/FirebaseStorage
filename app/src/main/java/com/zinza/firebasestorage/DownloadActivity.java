package com.zinza.firebasestorage;
...
public class DownloadActivity extends AppCompatActivity implements View.OnClickListener{
	...
	private StorageReference imageRef, imTets;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		imTets= FirebaseStorage.getInstance().getReferenceFromUrl("gs://fir-storage-5fa43.appspot.com").child("roboicon.png");
	...
	private void downloadDataViaUrl() {
		...
		try {
			...
			imTets.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
				@Override
...
//						mImageView.setImageBitmap(bitmap);
	...
	private void deleteFile() {
		Helper.showDialog(this);
		imTets.delete().addOnSuccessListener(new OnSuccessListener<Void>() {
			@Override
			public void onSuccess(Void aVoid) {
				Helper.dismissDialog();
				mImageView.setImageDrawable(null);
				mTextView.setText(String.format("%s was deleted.", imTets.getPath()));
			}
		}).addOnFailureListener(new OnFailureListener() {
			@Override
			public void onFailure(@NonNull Exception exception) {
